...
app.post("/reviews", async (req, res) => {
  const { spotifyId, reviewText, rating, userId } = req.body;

  try {
    const pool = await getConnection();

    await pool.request()
      .input("spotifyId", sql.NVarChar, spotifyId)
      .input("reviewText", sql.NVarChar, reviewText)
      .input("rating", sql.Int, rating)
      .input("userId", sql.Int, userId)
      .query(`
        INSERT INTO Reviews (SpotifyItemId, ReviewText, Rating, UserId)
        VALUES (@spotifyId, @reviewText, @rating, @userId)
      `);
