...
function renderAlbum(data: any) {
  ...
  setTimeout(() => {
    ...
    submitBtn.addEventListener("click", async () => {
      const reviewText = tinymce.get('review-text')?.getContent() || '';
      const rating = selectedRating;

      const spotifyId = albumData.id;
      const userId = localStorage.getItem("userId") || 1; // TEMP: mock user
      ...
      const payload = {
        ...
        rating,
        userId,
        type: "album" // or parsed.type dynamically
      };
      ...
      if (res.ok) {
        ...
        console.error("Error saving review:", errorData);
      }
    });
